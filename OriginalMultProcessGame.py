...
# Define the best_board pattern
# best_board = np.array([[0, 1, 0],
#                        [0, 0, 1],
#                        [1, 1, 1]])
...
# Create a larger pattern where each cell of the original pattern is replaced by the original pattern
bigger_board = np.block([[best_board]*size]*size)
print(bigger_board)

# Plot the resulting pattern
plt.imshow(bigger_board, cmap='binary')
plt.show()
# run the game of life from bigger, bored and evaluate the fitness of this the DQ an agent actually isn't needed at this portion, and that it could just be like the best of the best best boRDS of the best bpards
